<template>
  <div id="app" :style="{ height: windowHeight + 'px' }">
    ...
    <div style="display: flex; height: 100%; width: 100%; position: relative">
      <SideBar :data="data" style="height: 100%; position: fixed;" :style="{ height: windowHeight + 'px' }"></SideBar>
      <TextArea class="myTextArea" :style="{ height: windowHeight + 'px' }"></TextArea>
    </div>
    <div class="dialog text-white px-6 py-4 border-0 rounded bg-pink-500 z-50 shadow-lg transition-all" v-if="showAlert">
...
export default {
  ...
  setup () {
    ...
    const availableDialogType = { error: -1, newFileOrFolder: 0, renameFileOrFolder: 1, newTag: 2 }
    let dialogType = availableDialogType.error
    ...
    onMounted(async () => {
      ...
      bus.on('showDialogForNewFile', (obj) => {
        ...
        dialogType = availableDialogType.newFileOrFolder
        openModal()
      })
      ...
      bus.on('showDialogForRenameFile', (obj) => {
        ...
        father.value = obj
        dialogType = availableDialogType.renameFileOrFolder
        openModal()
      })

      bus.on('showDialogForNewTag', () => {
        dialogName.value = '转变为tag'
        dialogType = availableDialogType.newTag
        openModal()
      })
    ...
    function removeFileOrFolder (file) {
      closeTab(file)
      const { path } = file
      // 从后端删除
      if (file.type === 'file') {
    ...
    function nameValidTest (name) {
      return name.trim() && !name.includes(window.pathAPI.sep)
    }
    ...
    async function handleNew () {
      ...
      if (!nameValidTest(fileName.value)) {
        bus.emit('showAlertWith', { message: '文件名或文件夹名非法' })
        fileName.value = ''
      } else {
        if (father.value.children.find(stat => stat.name === fileName.value)) {
          bus.emit('showAlertWith', { message: '输入的名称已存在' })
          fileName.value = ''
          return
        }
        if (dialogName.value === '新建文件') {
        ...
        const obj = {
          name: fileName.value,
          path: window.pathAPI.join(father.value.path, fileName.value),
          children: [],
          curChild: -1,
          content: '',
          absolutePath: paths,
    ...
    function renameFile () {
      ...
      if (!nameValidTest(fileName.value)) {
        bus.emit('showAlertWith', { message: '请输入合法的文件名或文件夹名' })
        fileName.value = ''
      } else {
    ...
    function handle () {
      switch (dialogType) {
        case availableDialogType.newFileOrFolder:
          handleNew()
          break
        case availableDialogType.renameFileOrFolder:
          renameFile()
          break
        case availableDialogType.newTag:
          handleNewTag()
          break
        default:
    ...
    // 复制相对路径
    bus.on('setClipboardRelativePath', ({ path }) => {
      const relativePath = window.pathAPI.relative(data.value[0].path, path)
      navigator.clipboard.writeText(relativePath)
    })

    bus.on('showAlertWith', (obj) => {
      message.value = obj.message
      showAlert.value = true
    })

    /*
      move file/folder by drag
     */
    let dragBeginPath = ''
    let dragEndPath = ''
    bus.on('setDragBeginPath', ({ path }) => {
      dragBeginPath = path
    })

    bus.on('setDragEndPath', ({ type, path }) => {
      if (type === 'file') {
        dragEndPath = window.pathAPI.dirname(path)
      } else {
        dragEndPath = path
      }
    })
    ...
    bus.on('moveByDrag', () => {
      if (window.pathAPI.dirname(dragBeginPath) === dragEndPath) {
        return
      }
      window.electronAPI.moveFileOrFolder(dragBeginPath, dragEndPath)
    })
